...
# --- Monochrome handwriting theme colors (BGR format for Excel COM) ---
...
$colorBorder    = RGBtoExcel 192 192 192  # #C0C0C0  silver gray
...
try {
    ...
    # --- Reapply monochrome handwriting theme to ALL data rows after sort ---
    for ($r = 2; $r -le $lastRow; $r++) {
        ...
        for ($c = 1; $c -le 3; $c++) {
            ...
            $cell.Font.Size           = 10
            $cell.Font.Bold           = $true
            $cell.Font.Color          = $colorText
            $cell.Interior.Color      = $bg
            $cell.VerticalAlignment   = -4108  # xlCenter
            $cell.HorizontalAlignment = if ($c -eq 2) { -4108 } else { -4131 }
            $cell.Borders.Weight      = 2
            $cell.Borders.Color       = $colorBorder
        }
    }

    # --- Ensure header row keeps monochrome header style ---
    for ($c = 1; $c -le 3; $c++) {
